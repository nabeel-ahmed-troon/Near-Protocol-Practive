...
impl Airdrop for Lottery{
     ...
     fn ft_airdrop(&mut self)->Promise{
        ...
        if let Some(value)=self.claimers.get(&env::predecessor_account_id()){
            ...
            let t= ext_lottery_fungible_token::ft_transfer(env::predecessor_account_id(), _amount, Some("Airdrop for Lottery Participation".to_string()), "lottery_ft.testnet".to_string().parse().unwrap(), ONE_YOCTO,BASE_GAS)
            .then(
